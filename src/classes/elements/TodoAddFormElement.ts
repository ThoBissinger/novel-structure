...
// ---------------------------------------------------------------------------
// TodoAddModal's entire content — Roman/Private target picker, the field
// form, a staged (not-yet-persisted) subtask list, and the Add button.
// The subtask list stays a plain rebuild-on-change function rather than a
// reconciled element: it's a list of draft strings before the first save,
// not a diffable persisted entity list — id-keyed reconciliation would add
// ceremony for zero benefit here.
// ---------------------------------------------------------------------------

const TAG = "novel-todo-add-form-el";

export class TodoAddFormElement extends HTMLElement {
  private app!: App;
  private plugin!: NovelStructurePlugin;
  private targets: TodoTarget[] = [];
  ...
  private draw() {
    this.empty();
    this.createEl("h2", { text: "New todo" });
    ...
    if (this.targets.length === 1) {
      this.createEl("p", { text: this.targets[0].label, cls: "setting-item-description" });
    } else if (hasPrivateTarget) {
      const modeGroup = this.createDiv({ cls: "novel-structure-mode-group novel-todo-modal-mode-group" });
      const scenePickerWrap = this.createDiv();
      ...
      new Setting(this).setName("Where").addDropdown((dd) => {
    ...
    new Setting(this).setName("Text").addText((t) => {
      t.setPlaceholder("e.g. Sharpen the dialogue in act 2").onChange((v) => (this.text = v));
      t.inputEl.style.width = "100%";
      t.inputEl.focus();
    });

    new Setting(this).setName("Priority").addDropdown((dd) => {
    ...
    if (isPrivateOnly || hasPrivateTarget) {
      const recurrenceSetting = new Setting(this)
    ...
    new Setting(this).setName("Subtasks").setDesc("Optional. Break it down into concrete steps up front.");
    const subtaskList = this.createEl("div", { cls: "novel-todo-modal-subtask-list" });
    ...
    const subtaskAddRow = this.createEl("div", { cls: "novel-todo-modal-subtask-add-row" });
...
export function defineTodoAddFormElement(): void {
  if (defined || customElements.get(TAG)) return;
  customElements.define(TAG, TodoAddFormElement);
  defined = true;
}

export function createTodoAddFormElement(
  app: App,
  plugin: NovelStructurePlugin,
  parent: HTMLElement,
  targets: TodoTarget[],
  ...
  const el = document.createElement(TAG) as TodoAddFormElement;
  el.configure(app, plugin, targets, initialIndex, initialDeadline, closeModal, onDone);
