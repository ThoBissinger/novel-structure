...
//    (update-)import in any text mode) and "## Threads" (per-thread
//    development text for this scene, see threads.ts). Everything from the
//    first such heading onward is preserved verbatim across (update-)import,
//    no matter which text mode is used.
...
const TAIL_HEADINGS = [NOTES_HEADING, THREADS_HEADING];
...
/** Finds the ["## Threads", ...) line range within `tailLines` (a tail's
 * lines), i.e. up to (but excluding) the next top-level "## " heading or
 * end of tail. Returns null if no "## Threads" heading is present. */
function findThreadsRange(tailLines: string[]): { start: number; end: number } | null {
  const start = tailLines.findIndex((l) => l.trim() === THREADS_HEADING);
...
export function parseThreadDevelopments(body: string): Map<string, string> {
  ...
  const range = findThreadsRange(tailLines);
